...
### 練習問題 5.4
1. CRUD操作とは何かを、データリソースとの関係を使って説明してください。
  - 解答
    CRUD操作とはデータリソースのライフサイクルを実現する操作のこと。  
    Cはcreate => 新規作成  
    Rはread => 読み込み、取得  
    Uはupdate => 更新  
    Dはdestroy => 削除  
    を意味する。
  - 正解
    正解。モデルはデータベースとの連携により、テーブルのデータリソースをインスタンスとして実現し、データリソースに対する様々な指示を実行する役割がある。

2. CRUD操作を実現する代表的なメソッドについて、一つのモデルを例に具体的な使い方を説明してください。
  - 解答
    Cは、new,save,それらを一括処理するcreateがある。  
    `User.new(name: "名無しのごんべえ")`  
    `User.save`  
    上記のコードは`User.create(name: "名無しのごんべえ")`と同じ  
    ...
    `user.destroy`
  - 正解
    正解。newはインスタンス化、saveはインスタンスの保存を行うメソッド。  
    updateをモデルのクラスに対して使う場合は、  
    `User.update(1,{name: "ななしの権兵衛"})`とする。
    DはDeleteのD

3. save,create,updateのメソッドについて、役割、および関係をメソッドの対象となるオブジェクトの種類も含めて説明してください。
  - 解答
    saveはインスタンスの情報を保存するメソッドで、使用可能なオブジェクトはインスタンスのみでクラスオブジェクトに対しては使用できない。  
    createは逆にインスタンスオブジェクトに対して使用できないため、クラスオブジェクトに対して使用する。挙動はnewとsaveを一括処理したもの。  
    updateはインスタンス、クラスともに使用可能なメソッドである。情報を更新するメソッド。  
  - 正解
    正解。上記に加え、バリデーションに失敗した場合ロールバックし、createメソッドはインスタンスを返し、save/updateメソッドはfalseを返す。  
    この時Invalid例外エラーを発生させたい場合は、!をメソッド名の後ろにつける。

4. selectメソッドとfind_byメソッドの違い、およびselectとpluckメソッドの違いを具体的に説明してください。
  - 解答
    selectは複数取得可能なのに対し、find_byは最初の一件しか取得できない。
    selectはActiveRecord::Relationクラスが返ってくるのに対し、pluckメソッドはArrayクラスで返ってくる。
