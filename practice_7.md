...
1. アソシエーションとは何かを説明し、どのようなアソシエーションの関係があるかをピックアップしてください。
  ...
  - 正解
    - データベーステーブルに対して分離作業を行う正規化を用いて、モデル間の関係をシンプルかつスマートに実現する仕組み
2. アソシエーションを設定する基本の3つのメソッドについて、具体的なモデルで相互の設定方法を説明してください。
  ...
  - 正解
    - has_manyは複数の子モデルインスタンスを持つ
    - has_oneは一つの子モデルインスタンスを持つ
      - 解答の内容では家族モデルには`belongs_to :student`を記述するのが正解
    - belongs_toは一つの親モデルインスタンスに属する
...
3. 親子モデルのアソシエーションで、外部キーの役割と標準の設定ルールについて説明してください。
  ...
  - 正解
    - 親モデルIDを子モデルの属性として設定する。
    - 属性名は英小文字で「親モデル名_id」タイプはinteger
    - 他にも、子モデルに「親モデル名」タイプはreferencesタイプ　の方法もある。
...
4. 多対多の関係を作る2つの方法をそれぞれ具体的に説明してください。
  ...
  - 正解
    - hmt型
      - has_manyメソッドのthroughオプションを使用する方法
      - 書籍とユーザーがあり、貸し出し台帳が仲介モデルとなる。
      - 定義方法
        UserモデルとBookモデルと貸し出し台帳の仲介モデルがある時。
        - Userモデル
        ```ruby
        has_many :仲介モデル名複数
        has_many :books, through: :仲介モデル名複数
        ```
        - Bookモデル
        ```ruby
        has_many :仲介モデル名複数
        has_many :users, through: :仲介モデル名複数
        ```
    - habtm型
      - has_and_belongs_to_manyメソッドを使用する方法
      - 意味のある子モデルを介した結合ではなく、仲介・結合のためだけのテーブルを作成し、それを利用して関係つける。
      - idのみを保有する
      - 定義方法
        - BookモデルとUserモデルでメソッドを定義
        - マイグレーションファイルを作成する
...
6. N+1問題は、どのようなケースで発生するか、対処方法を含めて具体的に説明してください。
  ...
  - 正解
    - テーブルに対するn件のデータ検索を行う場合に、n+1件のSQL文が発行されることにより、処理速度が大幅に悪化する状況のこと
