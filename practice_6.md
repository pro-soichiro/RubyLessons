...
1. スコープの役割と設定場所について説明してください。
  - 解答
  よく使う条件検索機能をモデルに定義しておくことでメソットとして条件検索できる機能
  - 正解
  モデルのデータリソースの検索に便利な機能

2. 複数のスコープを組み合わせる方法について説明してください。
  - 解答
  メソッドチェーンを使って組み合わせることができる
  - 正解
  正解
3. デフォルトスコープがどのような場合に有効か、例をあげて説明してください。また、デフォルトスコープを無視する検索方法を説明してください。
  - 解答
  例えばUserモデルで未成年はデフォルトで取得しないようにする場合などに使用できる。
  age属性で年齢を入れておき
  ```ruby
  # モデルで
  default_scope -> { where("age < 20") }
...
モデルにはデフォルトの属性がある。  
例えば、created_atやidなど。  
その一つにlock_versionという属性がある。  
モデルを通して処理されるリソースに対して同時にアクセスがあったときに、更新の整合性を確保するために利用される属性  

### ロック機能の種類
1. 楽観的ロック機能  
  これはデータを読み込んだときのバージョンと更新するときのバージョンが同じかを比較し、もし他のユーザーが自分より先に更新した場合は更新できないようになる。  
  起きる可能性が低いため楽観的となっている。

2. 悲観的ロック機能  
  これはデータを取得した時点で他が読めないようにロックをかける。この場合は他のユーザーは、更新をかけることができず、待たされる。